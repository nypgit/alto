    ...
    implements Cipher 
{
    ...
    public final static OAEG OaegDefault = new SHACompressor();
     ...
     * @author John Pritchard
     */
    public final static class SHACompressor implements OAEG {
        ...
        public byte[] G ( OAEP oae, byte[] seed, int seedofs, int seedlen, int outlen){
            ...
            for ( int i = 0; i < loopbound; i++){
                ...
                if ( (maskofs+oae.H_length) < outlen)

                    java.lang.System.arraycopy(hbuf,0,mask,maskofs,oae.H_length);

                else {
                    int copymany = outlen-maskofs;

                    if ( 0 < copymany)
                        java.lang.System.arraycopy(hbuf,0,mask,maskofs,copymany);
    ...
    public OAEP ( java.util.Random R)
        throws java.security.NoSuchAlgorithmException
    {
        this("SHA1",R);
    }
    public OAEP ( String H, java.util.Random R)
        throws java.security.NoSuchAlgorithmException
    {
    ...
    public int encipher ( byte[] input, int inofs, int inlen, byte[] output, int outofs){
        ...
        if ( outlen > (outbuflen-outofs))

            throw new java.lang.IllegalArgumentException("Output buffer too short, requires (inlen+(2*hashlen ["+H_length+"])+1) bytes (after outofs ["+outofs+"]).");

        else {
            /*
             * This is working directly into the output buffer.
             * 
             * Overall output structure with offset arithmetic:
             *
             *    s     +     t
             *    |           |
             *    outofs      +inlen
            ...
            byte[] r = new byte[H_length];  rng.nextBytes(r);

            byte[] Gr = G.G(this,r,0,H_length,inlen);

            // s = (M xor G(r)) => output[outofs...(inlen-1)]

            for ( oc = outofs, ic = inofs, rc = 0; ic < inlen; ic++, rc++, oc++)

                output[oc] = (byte)((input[ic]&0xff)^(Gr[rc]&0xff));
            ...
            for ( oc = inlen, ic = 0, rc = 0; ic < H_length; oc++, ic++, rc++)
		
                output[oc] = (byte)((r[rc]&0xff)^(t[ic]&0xff));
    ...
    public int decipher ( byte[] input, int inofs, int inlen, byte[] output, int outofs){
         ...
         * This is working directly into the output buffer.
         * 
         * Overall input structure with offset arithmetic:
         *
         *    s     +     t
         *    |           |
         *    inofs       +slen
        ...
        for ( int ic = it, ir = 0; ir < H_length; ic++, ir++)

            r[ir] = (byte)((Hs[ir]&0xff)^(input[ic]&0xff));

        byte[] Gr = G.G(this,r,0,H_length,slen);
	
        for ( int oc = outofs, ic = inofs, ir = 0; ir < slen; oc++, ic++, ir++)

            output[oc] = (byte)((input[ic]&0xff)^(Gr[ir]&0xff));
