    ...
    extends Object 
{
    ...
    public final static int VERSION_3 = 3;

    private final static int keyBag[]  = {1, 2, 840, 113549, 1, 12, 10, 1, 2};
    private final static int certBag[] = {1, 2, 840, 113549, 1, 12, 10, 1, 3};

    private final static int pkcs9Name[]  = {1, 2, 840, 113549, 1, 9, 20};
    private final static int pkcs9KeyId[] = {1, 2, 840, 113549, 1, 9, 21};

    private final static int pkcs9certType[] = {1, 2, 840, 113549, 1, 9, 22, 1};

    private final static int pbeWithSHAAnd40BitRC2CBC[] =
                                        {1, 2, 840, 113549, 1, 12, 1, 6};
    private final static int pbeWithSHAAnd3KeyTripleDESCBC[] =
    ...
    private final static int SALT_LEN = 20;
    ...
    private Date date; // the creation date of this entry
    ...
    public PKCS12(String alias, PrivateKey key, String password, X509Certificate cert)
        throws IOException, CertificateException
    {
        this(alias,key,password.toCharArray(),new X509Certificate[]{cert});
    }
    public PKCS12(String alias, PrivateKey key, String password, X509Certificate[] chain)
        throws IOException, CertificateException
    {
     ...
     */
    public PKCS12(String alias, PrivateKey key, char[] password, X509Certificate[] chain)
        throws IOException, CertificateException
    {
        this.date = new Date();
        ...
        if ((keyFormat.equals("PKCS#8")) ||
            (keyFormat.equals("PKCS8")))
        {
            try {
                this.protectedPrivKey =
                    this.encryptPrivateKey(key.getEncoded(), password);
            }
     ...
     */
    public PKCS12(String alias, byte[] key, X509Certificate[] chain)
     ...
     */
    public PKCS12(InputStream stream, char[] password)
        throws IOException, CertificateException
    {
        super();
        this.decode(stream,password);
    }
    public PKCS12(InputStream stream, String password, boolean close)
        throws IOException, CertificateException
    {
        this(stream,password.toCharArray(),close);
    }
    public PKCS12(InputStream stream, char[] password, boolean close)
        throws IOException, CertificateException
    {
        super();
        try {
     ...
     * 
     */
    public void encode(OutputStream stream, String password)
        throws IOException, CertificateException
    {
        if (null != password)
            this.encode(stream,password.toCharArray());
        else
     ...
     */
    public void encode(OutputStream stream, char[] password)
        throws IOException, CertificateException
    {
        if (password == null) {
           throw new IllegalArgumentException("password");
     ...
     */
    public void decode(InputStream stream, char[] password)
        throws IOException, CertificateException
    {
        DataInputStream dis;
    ...
    public X509Certificate getCertificate(int idx){
        X509Certificate[] chain = this.chain;
        if (null != chain && -1 < idx && idx < chain.length)
            return chain[idx];
        else
            throw new java.lang.ArrayIndexOutOfBoundsException(String.valueOf(idx));
    }
    public PrivateKey getKey(String password)
        throws NoSuchAlgorithmException, InvalidKeyException, InvalidParameterSpecException
    {
        if (null != password)
            return this.getKey(password.toCharArray());
        else
        ...
        throws NoSuchAlgorithmException, InvalidParameterSpecException
    {
        PBEParameterSpec paramSpec =
            new PBEParameterSpec(this.getSalt(), ITERATION_COUNT);
        AlgorithmParameters algParams =
    ...
    private void validateMac(byte[] authSafeData, char[] password, DerInputStream s)
        ...
        throws IOException, CertificateException
    {
        ...
        // Wrap EncryptedPrivateKeyInfo in a context-specific tag.
        DerOutputStream bagValue = new DerOutputStream();
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (bagId.equals(PKCS8ShroudedKeyBag_OID)) {
                ...
                if (!certValues[1].isContextSpecific((byte)0)) {
                    throw new CertificateException("unsupported PKCS12 cert value type "
                                        + certValues[1].tag);
                }
