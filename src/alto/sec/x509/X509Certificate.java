    ...
    implements DerEncoder 
{
    ...
    private final static java.io.InputStream Markable(java.io.InputStream in){
        if (in instanceof java.io.ByteArrayInputStream)
            return in;
        else if (in instanceof java.io.BufferedInputStream)
            return in;
        else
     ...
     * @exception CertificateException on parsing and initialization errors.
     */
    public X509Certificate(DerValue derVal) 
        throws CertificateException 
    {
        this();
        try {
    ...
    public void encode(OutputStream out)
        throws java.security.KeyException, java.io.IOException
    {
        byte[] signedCert = this.signedCert;
        if (null == signedCert)
            throw new java.security.KeyException("Null certificate to encode");
        else {
    ...
    {
        if (this.readOnly)
            throw new CertificateException("Unable to overwrite certificate");
        else {
            in = Markable(in);
            try {
                /*
                 * Prepare Base64 encoded DER
                 */
     ...
     * @exception IOException on encoding error.
     */
    public void b64Encode(OutputStream out)
        throws java.io.IOException 
    {
        byte[] signedCert = this.signedCert;
        if (null == signedCert)
    ...
    public void sign(PrivateKey keyPrivate, X500Signer signer)
     ...
     */
    public static List<String> getExtendedKeyUsage(Certificate cert)
        throws CertificateException 
    {
        try {
     ...
     * provide among other things caching for better performance.
     */
    public static Collection<List<?>> getSubjectAlternativeNames(Certificate cert)
        throws CertificateException 
    {
        try {
            ...
            if (ext == null) {
                ...
                SubjectAlternativeNameExtension subjectAltNameExt =
                    new SubjectAlternativeNameExtension(Boolean.FALSE,data);
     ...
     * provide among other things caching for better performance.
     */
    public static Collection<List<?>> getIssuerAlternativeNames(Certificate cert)
        throws CertificateException 
    {
        try {
     ...
     * ASN.1 encoding.
     */
    private static X500Name getX500Principal(Certificate cert,
                                             boolean getIssuer) 
        throws Exception 
    {
     ...
     * the encoding.
     */
    public static X509Certificate toImpl(Certificate cert)
        throws CertificateException 
    {
        if (cert instanceof X509Certificate) {
     ...
     */
    public static boolean isSelfIssued(Certificate cert) 
         throws CertificateException 
    {
