...
public class DerValue {
    /**
     * Operators to shortcut the {@link DerOutputStream} for small,
     * internal tasks.
     */
    public static class Encode {
        public final static int LengthMark1 = (1 << 8);
        public final static int LengthMark2 = (1 << 16);
        public final static int LengthMark3 = (1 << 24);

        public final static byte LengthHead2 = (byte)0x081;
        public final static byte LengthHead3 = (byte)0x082;
        public final static byte LengthHead4 = (byte)0x083;
        public final static byte LengthHead5 = (byte)0x084;

        public final static byte[] Length(int len){
            if (len < 128) 
                return new byte[]{(byte)len};

            else if (len < LengthMark1){
                return new byte[]{
                    LengthHead2,
                    ((byte)len)
                };
            }
            else if (len < LengthMark2){
        ...
        public final static byte[] OctetString(byte[] string){
            int len = (null != string)?(string.length):(0);
            byte[] length = Length(len);
            int nlen = (1 + length.length + len);
            byte[] re = new byte[nlen];
            re[0] = DerValue.tag_OctetString;
            int ridx = 1;
            System.arraycopy(length,0,re,ridx,length.length);
            if (null != string){
                ridx += length.length;
                System.arraycopy(string,0,re,ridx,string.length);
            }
            return re;
        }
    }
