...
/**
 * This class implements the RFC822Name (email address string without
 * common name prefix or angle brackets) as required by the
 * GeneralNames ASN.1 object.
 * 
 * See RFC 5322 Section 3.4.1 (addr-spec).
 *
 * @author Amit Kapoor
    ...
    implements GeneralNameInterface
{
    ...
    /**
     * Validate an RFC822Name string to see if it is a valid addr-spec
     * according to IETF RFC822 and RFC2459:
     * <code>[local-part@]domain</code>
     * 
     * The <code>local-part@</code> could be empty for an RFC822Name
     * NameConstraint, but the domain at least must be non-empty.
        ...
        throws IOException 
    {
        if (name == null || 1 > name.length())
            ...
            if (1 > domain.length())
                ...
                throw new IOException("RFC822Name domain is '.'.");
        }
    }
