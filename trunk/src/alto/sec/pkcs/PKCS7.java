...
public class PKCS7 {
    ...
    throws ParsingException, IOException {
        ...
        for (int i=0; i < contents.length; i++) {
            ...
            try {
                if (certfac == null)
                    certificates[i] = new X509Certificate(contents[i]);
                else {
    ...
    private void parseSignedData(DerValue val)
        ...
        if ((byte)(dis.peekByte()) == (byte)0xA0) {
            ...
            for (int i = 0; i < len; i++) {
                ...
                try {
                    if (certfac == null)
                        certificates[i] = new X509Certificate(certVals[i]);
                    else {
        ...
        throws IOException
    {
        ...
        if (certificates != null && certificates.length != 0) {
            // cast to X509Certificate[] since X509Certificate implements DerEncoder
            X509Certificate implCerts[] = new X509Certificate[certificates.length];
            for (int i = 0; i < certificates.length; i++) {
                if (certificates[i] instanceof X509Certificate)
                    implCerts[i] = (X509Certificate) certificates[i];
                else {
                    try {
                        byte[] encoded = certificates[i].getEncoded();
                        implCerts[i] = new X509Certificate(encoded);
                    } catch (CertificateException ce) {
    ...
    public String toString() {
        ...
        out += contentInfo + "\n";
        if (version != null)
            out += "PKCS7 :: version: " + Hex.encode(version) + "\n";
